...
export default function SafeLogPage() {
  ...
  const [workCompleteSignatures, setWorkCompleteSignatures] = useState<{ [key: number]: string }>({});

  // 근로자 앱에서 작업 완료 서명 이벤트 수신
  useEffect(() => {
    const handleWorkCompleteSignature = (event: CustomEvent) => {
      const { workOrderId, signature } = event.detail;
      setWorkCompleteSignatures(prev => ({
        ...prev,
        [workOrderId]: signature
      }));
    };

    if (typeof window !== "undefined") {
      window.addEventListener("workCompleteSignature", handleWorkCompleteSignature as EventListener);
      return () => {
        window.removeEventListener("workCompleteSignature", handleWorkCompleteSignature as EventListener);
      };
    }
  }, []);
  ...
  const handleWorkComplete = (orderId: number) => {
    ...
    // 작업 완료 서명은 근로자 앱에서 이미 완료된 것으로 가정
    // 실제로는 근로자 앱에서 서명한 이미지 데이터를 받아옴
    const workCompleteSignature = workCompleteSignatures[orderId] || order.workCompleteSignature || null;
    ...
    const newEntry = {
      ...
      gpsLocation,
      signature: workCompleteSignature || `서명_${order.workerName}_${Date.now()}`,
    ...
    // 서명 이미지를 workCompleteSignatures에 저장 (이미 저장되어 있으면 그대로 사용)
    if (workCompleteSignature) {
      setWorkCompleteSignatures(prev => ({
        ...prev,
        [orderId]: workCompleteSignature
      }));
    }
          ...
          <CardContent>
            <div className="space-y-4 border rounded-lg p-6 bg-white">
              ...
              <div className="border-t pt-4">
                <div className="font-semibold mb-3">작업 종료 기록 및 서명</div>
                <div className="space-y-3">
                  {safeLog.workers.map((worker, idx) => {
                    // 작업 완료 서명 이미지 확인 (workCompleteSignatures에서 가져오거나 worker.signature에서)
                    const workOrder = completedOrders.find((o) => o.id === worker.workOrderId);
                    const signatureImage = workCompleteSignatures[worker.workOrderId] || 
                      (worker.signature && worker.signature.startsWith("data:image") ? worker.signature : null);
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          {signatureImage && (
                            <div className="border-2 border-gray-300 rounded p-1 bg-white flex-shrink-0" style={{ width: "120px", height: "50px" }}>
                              <Image 
                                src={signatureImage} 
                                alt={`${worker.workerName} 무재해 확인 서명`}
                                width={120}
                                height={50}
